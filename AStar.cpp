...
vector<Position> AStar::findPath(Position start, Position goal,
  ...
  openList.push({start, 0, calculateManhattan(start, goal),
  ...
  while (!openList.empty()) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (grid.isValid(neighbor.x, neighbor.y) &&
          !grid.isWall(neighbor.x, neighbor.y)) {
        int tentativeG = current.g + 1;

        if (gScore.find({neighbor.x, neighbor.y}) == gScore.end() ||
          ...
          if (isStartNode) {
            ...
            cout << "  g = " << tentativeG << " | h = " << h << " | f = " << f
                 << "\n";
            cout << "  Grid Preview:\n";
            GridDisplay::print(grid, goal, neighbor, vaultCollected, turn);
            cout << "\n";
          }
        }
      }
